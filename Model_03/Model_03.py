# Attempts to make predictions into the future based on previously day-by-day prediction progression
...
real_stock_price = dataset_test.iloc[timestep_size:, 1:2].values
...
inputs = dataset_total[len(dataset_train):].values
...
for k in range(0, 100):  
    ...
    # Making predictions and adding them to the list of future predictions
    predicted_stock_price = model.predict(X_test)
    latest_value = predicted_stock_price[len(predicted_stock_price) - 1]
    future_stock_price.append(latest_value)
    
    # Reset testing data sets for next iteration
    dataset_total = pd.concat((dataset_train['Open'], dataset_test['Open']), axis = 0)
    
    inputs = dataset_total[len(dataset_train):].values
    inputs = inputs.reshape(-1,1)
    inputs = sc.transform(inputs)
    inputs = np.append(inputs, future_stock_price)
    inputs = inputs.reshape(-1,1)
...
# Reversing normalization of predicted values
